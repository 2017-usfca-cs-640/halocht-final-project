---
title: "Final Report: Life After Death- The Prevailing Microbial Species of Recently-Deceased Individuals"
author: "Alfredo Hans Locht"
date: "November 29, 2017"
output:
    github_document: default
    word_document: default
bibliography: references.bib
csl: bioinformatics.csl
---
```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library("dplyr")
library("tidyr")
library("knitr")
library("ggplot2")
library("png")
load("output/Thanatomicrobiome_phyloseq.RData")

```

# Introduction

Add about 2-3 pages here. Across the whole manuscript, you should cite at least 20 peer reviewed articles.

# Methods

## Study design

[@Hyde2013] obtained cadavers donated to the Southeast Texas Applied Forensic Science (STAFS) Facility at the Center for Biological Field Studies (CBFS) at Sam Houston State University (SHSU). Both CBFS and STAFS are located in a humid, subtropical climate characterized by a large distribution of pine trees and acidic soils in an area 5 km North of Huntsville, Texas. STAFS is a sectioned-off, two-acre area within the larger, 243 acre CBFS complex which is covered with pine trees and herbaceous plants. The STAFS facility is considered to be a “willed donations” center, so donors or family members of the deceased are required to sign a form in which they indicate that they understand that their donated corpse will be used for scientific research and teaching purposes. 

This form also included information regarding the methods that would be used to study the cadavers. Specifically, the study informed the donors and/or next of kin that the body would be left outside to decompose in natural and aerobic conditions, and that samples pertaining to different types of research (bacterial, tissue, and entomological) would be acquired at different stages of the decomposition for an allotted period of time. Upon request, the family members could also choose to be debriefed about the scientific knowledge or educational project that was achieved during the course of the study and as a result of the donated cadaver. 

## Sample origin and sequencing

### Cadaver Demographics

The two cadavers donated to the center that were studied, STAFS 2011-016 and STAFS 2011-006, were both males of ages 68 and 52 respectively, of Caucasian descent, were corpses that had not been autopsied, and were considered to be in the early stages of decomposition as neither showed signs of bloat [@Hyde2013]. STAFS 2011-016 had a previous medical history relating to the cause of death (acute myocardial infarction), been frozen for 89 days, and had been subsequently defrosted in a fridge cooler for 143 days before being placed outside to decompose. STAFS 2011-006 did not have any pertaining medical history other than the cause of death (carbon monoxide poisoning), had been frozen for 13 days, and had subsequently defrosted for 6 days in a fridge cooler before decomposition placement. The time since death was unknown for both cadavers. 

[@Hyde2013] further explains that because of the inherent randomness of the STAFS willed donations program, variables surrounding the acquisition and previous care of the cadavers were strikingly dissimilar between cadavers, but could not be controlled for. Other variables speaking towards the physical characteristics such as height and weight of each male cadaver were unknown, but estimated to be of average height and above-average weight in comparison to an average American male. The authors sought to reduce other possible confounding variables to the best of their knowledge [@Hyde2013].

### Sample Acquisition

### Inherent and Inevitable Variability

Conditions surrounding the death of any given cadaver can have high variability in confounding factors which can directly influence the rate and pattern of decomposition [@Bates2016; @Cockle2015; @Ferreira2013; @Hauther2015; @Hyde2013; @Vass2011]. Intrinsic factors such as cause of death, physical trauma, and postmortem care can directly interact with a number of other, extrinsic factors such as temperature and humidity of environment in manners which have not yet been efficiently characterized [@Cockle2015]. [@Hyde2013] forewarn that because of the inherent inconsistency of the STAFS willed donations program (cadavers can be obtained on a random or unexpected basis) and the associated complications in conducting studies on cadavers, data acquired for both cadavers cannot be compared directly but rather, should act as a complimentary set of data. Although the authors sought to control or reduce the amount of confounding variables to the best of their knowledge, they were able to identify some of the limiting qualities of their study. These disclaimers are investigated below: 

In order to establish a baseline of decomposition of both cadavers, they were initially qualified by a visual scoring system developed by [@Megyesi2005]. A summary of the system includes visible cues that might indicate at what step of the decomposition process a particular cadaver may be at. Using this qualitative method, both cadavers were considered to be in the early stages of decomposition as no bloat had occurred This system was used to calculate PMI and directly relies on temperature  for assessing the end of   Other variables speaking towards the physical characteristics such as height and weight of each male cadaver were unknown, but estimated to be of average height and above-average weight in comparison to an average American male. The authors sought to control or reduce other possible confounding variables to the best of their knowledge [@Hyde2013]:

One manner in which they were able to control the temperature of decomposition was through the assignation of Accumulated Degree-Days (ADD) which quantify the amount of heat-energy units 

## Computational

These are the methods you used to do your bioinformatic analyses analyses. Should probably be between 0.5 and 1 pages. At a very minimum should include citations for DADA2 and phyloseq if you are doing an amplicon study, or other citations as appropriate.

# Results

```{r}
plot_richness(phyloseq_obj,
              x = "collection_date_s",
              measures = c("Shannon", "Simpson"),
              color = "sample_description_s") +
  xlab("Source of Sample") +
  geom_jitter(width = 0.2) +
  theme(axis.text.x = element_text(angle = 90,
                                   hjust = 1))
  theme_bw()
```

```{r}
plot_richness(phyloseq_obj,
              x = "sample_description_s",
              measures = c("Shannon", "Simpson"),
              color = "collection_date_s") +
  xlab("Source of Sample") +
  geom_jitter(width = 0.2) +
  theme(axis.text.x = element_text(angle = 90,
                                   hjust = 1))
  theme_bw()
```

```{r}
gp_ch <- subset_taxa(phyloseq_obj, Phylum == "Firmicutes")
plot_bar(gp_ch,
  fill = "Family",
  title = "Firmicutes Phyla Acrosss Distribution Across Samples")
```

```{r}
plot_richness(gp_ch,
              x = "sample_description_s",
              measures = c("Shannon", "Simpson"),
              color = "collection_date_s") +
  xlab("Source of Sample") +
  geom_jitter(width = 0.2) +
  theme(axis.text.x = element_text(angle = 90,
                                   hjust = 1))
  theme_bw()
```





```{r}
gp_ch2 <- subset_taxa(phyloseq_obj, Phylum == "Actinobacteria")
plot_bar(gp_ch2,
  fill = "Family",
  title = "Actinobacteria Phyla Acrosss Distribution Across Samples")
```


```{r}
gp_ch3 <- subset_taxa(phyloseq_obj, Class = "Chlostridia")
plot_bar(gp_ch3,
  fill = "Order",
  title = "Firmicutes Phyla Acrosss Distribution Across Samples")
```

```{r}
gpac <- subset_taxa(phyloseq_obj, Phylum == "Firmicutes")
gpac <- prune_taxa(names(sort(taxa_sums(gpac),TRUE)[1:300]), gpac)
plot_heatmap(phyloseq_obj,
             method = "NMDS"
             )









```











## Subsections are ok in the results section too

```{r your-code-here}
# Add code chunks as needed for your analyses
# For most analyses, I would recommend splitting the
# intensive computational part into a seperate R script file
# and then just load your libraries and the data object here
# using the `load()` function. Ask me for clarification if this is
# unclear.
```

In addition to a minimum of 5-10 figures/tables (and associated captions), you should include sufficient text in this section to describe what your findings were. Remember that in the results section you just describe what you found, but you don't interpret it - that happens in the discussion. 2-3 pages.

# Discussion

Add around 3-4 pages interpreting your results and considering future directions one might take in analyzing these data.

# Sources Cited
